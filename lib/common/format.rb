...
module Format
  ...
  def self.float_to_percent(value, precision = 0, right_padding = 4)
    format("%#{right_padding}.#{precision}f %", value.to_f * 100.0)
  end

  # Converts a +Float+ value to a +String+ using variable precision format
  #
  def self.float_value(value, precision = 2, right_padding = 0)
    format("%#{right_padding}.#{precision}f", value.to_f)
  end
  # ---------------------------------------------------------------------------
  #++

  # RegExp used to search for Integer-like values in formatted strings.
  INTEGER_STRING_SEARCH_REGEXP  = /(^|\s+)\-*\d+(\s+|$)/.freeze unless defined? INTEGER_STRING_SEARCH_REGEXP

  # RegExp used to search for Float-like values in formatted strings.
  FLOAT_STRING_SEARCH_REGEXP    = /(^|\s+)\-*\d+((\.|\,)\d+(e(\+|\-)\d+)?)?(\s+|$)/.freeze unless defined? FLOAT_STRING_SEARCH_REGEXP

  # RegExp used to search for Date-like values in formatted strings.
  DATE_STRING_SEARCH_REGEXP = %r{(^|\s+)(\d{1,4}(\-|/|\.)\d{1,2}(\-|/|\.)\d{1,4})\s*\w*\s*$}.freeze unless defined? DATE_STRING_SEARCH_REGEXP
  ...
  def self.could_be_an_integer?(an_object)
    return true if an_object.is_a?(Integer)
    ...
    # DEBUG
    # puts "checking could_be_an_integer? '#{an_object}' for pattern match => " \
    #      "#{ an_object.to_s =~ INTEGER_STRING_SEARCH_REGEXP }"
    !(an_object.to_s =~ INTEGER_STRING_SEARCH_REGEXP).nil?
  end
  ...
  def self.could_be_a_float?(an_object)
    return true if an_object.is_a?(Float)
    ...
    # DEBUG
    # puts "checking could_be_a_float? '#{an_object}' for pattern match => " \
    #      "#{ an_object.to_s =~ FLOAT_STRING_SEARCH_REGEXP }"
    !(an_object.to_s =~ FLOAT_STRING_SEARCH_REGEXP).nil?
  end
  ...
  def self.could_be_a_date?(an_object)
    return true if an_object.is_a?(Date)

    # DEBUG
    # puts "checking could_be_a_date? '#{an_object}' for pattern match => " \
    #      "#{ an_object.to_s =~ DATE_STRING_SEARCH_REGEXP }"
  ...
  def self.could_be_a_time?(an_object)
    return true if an_object.is_a?(Time)
    ...
    # DEBUG
    # puts "checking could_be_a_time? '#{an_object}' for pattern match => " \
    #      "#{ an_object.to_s =~ TIME_STRING_SEARCH_REGEXP }, parsing: #{ is_parsing_ok} (#{parsed_value})"
    is_parsing_ok &&
  ...
  def self.could_be_a_datetime?(an_object)
    return true if an_object.is_a?(DateTime)
    ...
    # DEBUG
    # puts "checking could_be_a_datetime? '#{an_object}' for pattern match => " \
    #      "#{ an_object.to_s =~ TIME_STRING_SEARCH_REGEXP }, parsing: #{ is_parsing_ok} (#{parsed_value})"
    is_parsing_ok &&
  ...
  # - float_point_char : char to be used as floating point separator (useful for CSV automatic
  #   text import in external applications, which rely on current locale)
  #
  ...
  def self.to_csv_line(array_of_values, separator)
    ...
    array_of_values.collect!(&:to_s).compact.join(separator) << "\r\n"
  end
  ...
  def self.data_table_to_csv(ruport_table, csv_separator)
    ...
    ruport_table.each do |rec_row|
      s_a_result = []
      ruport_table.column_names.each do |name|
  ...
  def self.any_datetime(datetime_value, str_format)
    ...
    if datetime_value.is_a?(Time) || datetime_value.is_a?(Date) || datetime_value.is_a?(DateTime)
