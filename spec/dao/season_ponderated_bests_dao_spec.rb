...
describe SeasonPonderatedBestsDAO, type: :model do
  ...
  let(:pool_type)           { PoolType.find_by(code: '25') } # FIXME: Randomize

  let(:sure_event_type)     { EventType.find_by(code: '50SL') }
  let(:sure_pool_type)      { PoolType.find_by(code: '25') }
  let(:sure_category_type)  { season.category_types.find_by(code: 'M25') }

  let(:empty_event_type)    { EventType.find_by(code: '100MI') }
  let(:empty_pool_type)     { PoolType.find_by(code: '50') }
  let(:empty_category_type) { season.category_types.find_by(code: 'M50') }
  ...
  context 'EventPonderatedBestDAO subclass,' do
    ...
    it_behaves_like('(the existance of a method)', [:season, :season_type, :gender_type, :category_type, :event_type, :pool_type, :best_results, :total_results, :get_max_results, :get_bests_to_be_ignored, :collect_event_bests, :set_ponderated_best, :get_ponderated_best])
  ...
  context 'as a valid instance,' do
    ...
    it_behaves_like('(the existance of a method)', [:season, :single_events, :insert_events, :update_events, :max_results, :bests_to_be_ignored, :event_types, :categories, :find_season_type_events, :find_season_type_category_codes, :scan_for_gender_category_and_event])
    ...
    describe '#categories' do
      it 'is a collection of category codes' do
        ...
        found_cat.each do |category_code|
          expect(CategoryType.find_by(code: category_code)).to be_an_instance_of(CategoryType)
        end
      end
    end
