...
describe SeasonCreator, type: :strategy do
  context 'with non-valid parameters,' do
    it 'raises an exception for wrong season parameter' do
      ...
      expect { SeasonCreator.new(Season.all.limit(100).min { 0.5 - rand }) }.to raise_error(ArgumentError)
  ...
  context 'with valid parameters,' do
    ...
    let(:older_season)    { SeasonType.find_by(code: 'MASCSI').seasons.last }
    ...
    it_behaves_like('(the existance of a method)', [:description, :new_id, :begin_date, :end_date, :header_year, :edition, :categories, :meetings, :meeting_sessions, :meeting_events, :renew_season!, :renew_categories!, :renew_meetings!])
  ...
  describe '#renew_categories!,' do
    ...
    it 'returns the same category types code of older season' do
      subject.each do |category_type|
        expect(older_season.category_types.find_by(code: category_type.code)).to be_an_instance_of(CategoryType)
      end
    end
