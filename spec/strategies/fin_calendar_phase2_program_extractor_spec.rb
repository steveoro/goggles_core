...
describe FinCalendarPhase2ProgramExtractor, type: :strategy do
  ...
  describe "self.extract_from_nokogiri_nodeset" do
    ...
    context "when given fixture file 'man20131123innsbruckmastershark'," do
      let(:html_doc) do
        File.open( File.join( Rails.root, 'spec/fixtures/samples/man20131123innsbruckmastershark.html' ) ).read
      end
...
#        puts "\r\nresult:\r\n------------8<------------\r\n#{ result }\r\n------------8<------------"
    ...
    context "when given fixture file 'man20140201reglazio'," do
      let(:html_doc) do
        File.open( File.join( Rails.root, 'spec/fixtures/samples/man20140201reglazio.html' ) ).read
      end
      let(:nokogiri_nodeset)      { Nokogiri::HTML( html_doc ).css('#content') }
...
#        puts "\r\nresult:\r\n------------8<------------\r\n#{ result }\r\n------------8<------------"
    ...
    context "when given fixture file 'man20150214regcampania'," do
      let(:html_doc) do
        File.open( File.join( Rails.root, 'spec/fixtures/samples/man20150214regcampania.html' ) ).read
      end
      let(:nokogiri_nodeset)      { Nokogiri::HTML( html_doc ).css('#content') }
...
#        puts "\r\nresult:\r\n------------8<------------\r\n#{ result }\r\n------------8<------------"
    ...
    context "when given fixture file 'man20160207regpuglia'," do
      let(:html_doc) do
        File.open( File.join( Rails.root, 'spec/fixtures/samples/man20160207regpuglia.html' ) ).read
      end
      let(:nokogiri_nodeset)      { Nokogiri::HTML( html_doc ).css('#content') }
...
#        puts "\r\nresult:\r\n------------8<------------\r\n#{ result }\r\n------------8<------------"
    ...
    context "when given fixture file 'man20160220regcampania'," do
      let(:html_doc) do
        File.open( File.join( Rails.root, 'spec/fixtures/samples/man20160220regcampania.html' ) ).read
      end
      let(:nokogiri_nodeset)      { Nokogiri::HTML( html_doc ).css('#content') }
