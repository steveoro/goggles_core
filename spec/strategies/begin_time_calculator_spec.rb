...
describe BeginTimeCalculator, type: :strategy do
  ...
  describe 'self.compute_from_previous' do
    ...
    context 'when given a previous begin time,' do
      let(:previous_begin_time) { Time.zone.parse("#{scheduled_date_string} #{ format('%02d', (8 + rand * 9).to_i) }:00") }
  ...
  describe 'self.compute_for_all' do
    ...
    let( :meeting_with_entries)     { Meeting.find(14_101) }
    let( :meeting_with_results)     { Meeting.find(13_101) }
