...
describe FinCalendarMeetingBuilder, type: :strategy do
  ...
  let( :matching_meeting_ids )  { [16_232, 16_215, 16_236, 16_358, 16_201, 16_230, 16_237, 16_300, 16_247] }
  ...
  context 'with valid parameters,' do
    ...
    describe 'self.get_month_number()' do
      let(:months)            { %w[gennaio febbraio marzo aprile maggio giugno luglio agosto settembre ottobre novembre dicembre] }
      let(:random_month)      { months.sample }
      let(:random_int_month)  { (1..12).map(&:to_s).sample }
        ...
        context "for a valid day text (#{ fixture_day })," do
          ...
          it 'is properly formatted' do
            expect(
              ...
              format('%04d-%02d-%02d', random_year, months.index(random_month)+1, fixture_day.split(/\-/).first)
            )
          end
        end
      end
    ...
    describe 'self.has_different_values?()' do
      let(:random_meeting_read_only) do
        meeting = Meeting.all.sample
        meeting.do_not_update = true
        meeting
      end
      let(:random_meeting_updatable) do
        meeting = Meeting.all.sample
