...
#          subject.report
          expect( subject.result_meeting_events ).to be_an( Array )
          expect( subject.result_meeting_events.map{ |row| row.id } ).to match_array( existing_meeting_session.meeting_events.to_a.map{ |row| row.id } )
          expect( subject.result_meeting_events.map{ |row| row.meeting_session_id }.uniq.first ).to eq( existing_meeting_session.id )
...
#          end
      ...
      context "when called for a NOT-yet existing MeetingEvent," do
        let( :already_defined_types ) { existing_meeting_session.meeting_events.map{ |me| me.event_type_id } }
...
#          puts "\r\n- :dao_for_new => #{ dao }"
        ...
        it "returns a MeetingEvent instance" do
          ...
          expect( subject.result_meeting_events ).to be_an( Array )
          expect( subject.result_meeting_events.map{ |event| event.event_type_id } ).to include( new_event_type.id )
        end
