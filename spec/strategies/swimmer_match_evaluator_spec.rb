...
describe SwimmerMatchEvaluator, type: :model do
  ...
  describe '#set_visitor' do
    ...
    it "doesn't set visitor swimmer if wrong swimmer parameter" do
      ...
      wrong_gender_swimmer.gender_type = swimmer.gender_type.code == 'M' ? GenderType.find_by(code: 'F') : GenderType.find_by(code: 'M')
      expect(subject.visitor_swimmer).to be nil
  ...
  describe '#has_matches_on_event?' do
    ...
    it 'returns true for Leega and Steve on 100MI, 50FA and 100SL' do
      sme = SwimmerMatchEvaluator.new(@leega)
      sme.set_visitor(@steve)
      expect(sme.has_matches_on_event?(EventType.find_by(code: '100MI'))).to be true
      expect(sme.has_matches_on_event?(EventType.find_by(code: '50FA'))).to be true
      expect(sme.has_matches_on_event?(EventType.find_by(code: '100SL'))).to be true
    end
    it 'returns false for Leega and Steve on 400MI and 1500SL' do
      sme = SwimmerMatchEvaluator.new(@leega)
      sme.set_visitor(@steve)
      expect(sme.has_matches_on_event?(EventType.find_by(code: '400MI'))).to be false
      expect(sme.has_matches_on_event?(EventType.find_by(code: '1500SL'))).to be false
    end
  end
  #-- -------------------------------------------------------------------------
  ...
  describe '#get_matches_on_event' do
    ...
    it 'returns nil if no matches of given event type' do
      ...
      expect(sme.has_matches?).to be true
      expect(sme.has_matches_on_event?(EventType.find_by(code: '400MI'))).to be false
      expect(sme.get_matches_on_event(EventType.find_by(code: '400MI'))).to be nil
      expect(sme.has_matches_on_event?(EventType.find_by(code: '1500SL'))).to be false
      expect(sme.get_matches_on_event(EventType.find_by(code: '1500SL'))).to be nil
    end
    it 'returns a set of meeting programs for Leega and Steve including only given event type' do
      sme = SwimmerMatchEvaluator.new(@leega)
      sme.set_visitor(@steve)
      matches = sme.get_matches_on_event(EventType.find_by(code: '100MI'))
      expect(matches).to be_a_kind_of(ActiveRecord::Relation)
      expect(matches.count).to be > 0
      expect(matches).to all(be_an_instance_of(MeetingProgram))
      events = matches.map { |mp| mp.event_type.code }.uniq
      ...
      matches = sme.get_matches_on_event(EventType.find_by(code: '100SL'))
      expect(matches).to be_a_kind_of(ActiveRecord::Relation)
      expect(matches.count).to be > 0
      expect(matches).to all(be_an_instance_of(MeetingProgram))
      events = matches.map { |mp| mp.event_type.code }.uniq
      ...
      matches = sme.get_matches_on_event(EventType.find_by(code: '50FA'))
      expect(matches).to be_a_kind_of(ActiveRecord::Relation)
      expect(matches.count).to be > 0
      expect(matches).to all(be_an_instance_of(MeetingProgram))
      events = matches.map { |mp| mp.event_type.code }.uniq
