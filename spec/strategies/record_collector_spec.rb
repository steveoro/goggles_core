...
describe RecordCollector, type: :strategy do
  ...
  context '[implemented methods]' do
    it_behaves_like('(the existance of a method)',
                    [:swimmer, :team, :season, :season_type, :meeting, :collection, :count, :clear, :collect_from_results_having, :collect_from_all_category_results_having, :collect_from_records_having, :get_collected_season_types, :save, :commit, :full_scan, :sql_executable_log])
    it_behaves_like('(the existance of a method returning an Enumerable of non-empty Strings)',
                    [:record_type_code_list, :pool_type_code_list, :event_type_codes_list, :category_type_codes_list, :gender_type_codes_list])
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  describe '#collect_from_results_having' do
    ...
    context 'with a subject filtered by MEETING,' do
      let(:meeting)   { Meeting.has_results.min { rand - 0.5 } }
    ...
    context 'with a subject filtered by TEAM,' do
      let(:team)      { Team.has_results.min { rand - 0.5 } }
  ...
  context 'when filtering with a :meeting,' do
    ...
    describe '#pool_type_code_list' do
      it 'returns the pool_type codes of the sessions from the meeting used as filter' do
        ...
        ).to match_array(meeting.pool_types.to_a.flatten.map(&:code).flatten.uniq)
      end
    end
    ...
    describe '#event_type_codes_list' do
      it 'returns the event_type codes of the sessions from the meeting used as filter' do
        expect(
          ...
          meeting.event_types.are_not_relays.to_a.flatten.map(&:code).flatten.uniq
        )
      end
    end
    ...
    describe '#category_type_codes_list' do
      it 'returns the category_type codes of the sessions from the meeting used as filter' do
        expect(
          ...
          meeting.meeting_events.to_a.flatten.uniq.map do |me|
            me.category_types.to_a.flatten.map(&:code).uniq
          end.flatten.uniq
        )
      end
    end
  ...
  describe '#seek_existing_record_for' do
    context 'when searching for an existing FEDERATION record,' do
      let(:existing_ind_record) { IndividualRecord.where(is_team_record: false).limit(1000).all.min { rand - 0.5 } }
      let(:test_subject) do
    ...
    context 'when searching for an existing TEAM record,' do
      let(:existing_ind_record) { IndividualRecord.where(is_team_record: true).limit(1000).all.min { rand - 0.5 } }
      let(:test_subject) do
    ...
    context 'when searching for a missing FEDERATION record,' do
      let(:missing_ind_record) do
        fixture_row = IndividualRecord.where(is_team_record: false).limit(1000).all.min { rand - 0.5 }
    ...
    context 'when searching for a missing TEAM record,' do
      let(:missing_ind_record) do
        fixture_row = IndividualRecord.where(is_team_record: true).limit(1000).all.min { rand - 0.5 }
        fixture_row.pool_type_id = PoolType::MT33_ID
        fixture_row
      end
      let(:test_subject) do
