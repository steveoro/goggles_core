...
describe TeamBestFinder, type: :strategy do
  ...
  let(:season_type) do
    st = SeasonType.find_by(code: %w[MASCSI MASFIN].sample)
    expect(st).to be_a(SeasonType)
    st
  end
  let(:season) do
    s = Season.has_results.for_season_type(season_type).sample
    expect(s).to be_a(Season)
    s
  end
  ...
  let(:active_team)     { season.teams.has_many_results.count > 0 ? season.teams.has_many_results.sample : fix_team }
  let(:gender)          { GenderType.individual_only.sample }
  let(:pool)            { PoolType.only_for_meetings.sample }
  let(:event)           { EventType.are_not_relays.for_fin_calculation.sample }
  let(:category) do
    c = CategoryType.are_not_relays.for_season(season).sample
    expect(c).to be_a(CategoryType)
    c
  end
  let(:fin_season) do
    fs = Season.for_season_type(SeasonType.find_by(code: 'MASFIN')).sample
    expect(fs).to be_a(Season)
    expect(fs.season_type.code).to eq('MASFIN')
    fs
  end
  let(:fix_mir) { active_team.meeting_individual_results.is_not_disqualified.sample }
  #-- -------------------------------------------------------------------------
  #++
  ...
  context 'with valid parameters' do
    it_behaves_like('(the existance of a method)', [:team, :distinct_categories, :gender_types, :pool_types, :event_types, :set_genders, :set_pools, :set_events, :retrieve_distinct_categories, :find_category_by_code, :category_needs_split?, :get_category_to_split_into, :has_individual_result?, :get_team_best_individual_result, :scan_for_distinct_bests, :get_categories_to_split, :split_categories])
    ...
    describe '#gender_types,' do
      it 'are the default pool types' do
        ...
        expect(subject.gender_types).to include(GenderType.find_by(code: 'M'))
        expect(subject.gender_types).to include(GenderType.find_by(code: 'F'))
      end
    end
    ...
    describe '#pool_types,' do
      it 'are the default pool types' do
        ...
        expect(subject.pool_types).to include(PoolType.find_by(code: '25'))
        expect(subject.pool_types).to include(PoolType.find_by(code: '50'))
      end
    end
    ...
    describe '#event_types,' do
      it 'are the default event types' do
        ...
        expect(subject.event_types).to include(EventType.find_by(code: '100MI'))
        expect(subject.event_types).to include(EventType.find_by(code: '1500SL'))
        expect(subject.event_types).not_to include(EventType.find_by(code: '25SL'))
      end
    end
    #-- -----------------------------------------------------------------------
    #++
    ...
    describe '#category_needs_split?,' do
      ...
      xit 'returns false for standard FIN categories for multiple season types' do
        ...
        # XXX This one seem to work just fine:
        fin_categories = fin_season.category_types.are_not_relays.to_a.select { |row| row.code =~ /M??/ }
        fin_category = fin_categories.min { rand - 0.5 }
        # .sample
        # DEBUG
      ...
      it 'returns true for non standard FIN categories (U25) for multiple season types' do
        ...
        u25_category = fin_season.category_types.are_not_relays.where("code like 'U%'").sample
      ...
      it 'returns true for CSI grouped categories (OVER) for multiple season types' do
        over_category = CategoryType.find_by(code: 'OVER')
    ...
    describe '#get_category_to_split_into,' do
      ...
      it 'returns the same category if split not needed' do
        ...
        m45_mir = fix_team.meeting_individual_results.for_category_code('M45').sample
        ...
        m30_mir = fix_team.meeting_individual_results.for_category_code('M30').sample
      ...
      it 'returns a different category if split needed' do
        ...
        over_mir = fix_team.meeting_individual_results.for_category_code('OVER').sample
        ...
        s50_mir = fix_team.meeting_individual_results.for_category_code('50S').sample
    ...
    describe '#has_individual_result?,' do
      ...
      it 'returns true for event with results' do
        ...
        fix_mir = active_team.meeting_individual_results.is_not_disqualified.sample
    ...
    describe '#scan_for_distinct_bests,' do
      ...
      before(:each) do
        ...
        @new_team.meeting_individual_results.is_not_disqualified.each do |mir|
          @new_tbf.distinct_categories << mir.category_type unless @new_tbf.distinct_categories.rindex { |e| e.code == mir.category_type.code }
        end
    ...
    describe '#split_categories,' do
      ...
      before(:all) do
        ...
        @category_to_split = @new_tbf.get_categories_to_split.map(&:code)
      ...
      it 'returns a RecordX4dDAO with splitted category records correctly managed' do
        @records_to_split.each do |record_to_split|
          ...
          if %w[50S M20 SEN].include?(record.category_type.code)
            # DEBUG
