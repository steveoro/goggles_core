...
describe User, type: :model do
  ...
  context '[a well formed instance]' do
    ...
    context '[general methods]' do
      it_behaves_like('(the existance of a method)', [:get_full_name, :to_s, :get_first_and_last_name, :get_preferred_swimmer_level_id, :get_swimmer_level_type, :set_associated_swimmer, :has_associated_swimmer?, :has_swimmer_confirmations?, :find_any_confirmation_given_to])
    end

    # Filtering scopes:
    it_behaves_like('(the existance of a scope with no parameters)', [:data_updates_newsletter_readers, :achievements_newsletter_readers, :generic_newsletter_readers, :community_newsletter_readers])
    #-- -----------------------------------------------------------------------
    #++
    ...
    describe '#find_team_affiliation_id_from_team_managers_for' do
      let(:random_season) { Season.all.min { 0.5 - rand } }
      let(:random_team_manager) { TeamManager.all.min { 0.5 - rand } }
