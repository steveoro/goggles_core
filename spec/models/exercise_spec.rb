...
describe Exercise, type: :model do
  context '[a well formed instance]' do
    subject { Exercise.all.min { rand - 0.5 } }
    ...
    context '[implemented methods]' do
      it_behaves_like('(the existance of a class method)',
                      [:belongs_to_training_step_code, :get_label_symbol])
      it_behaves_like('(the existance of a method)',
                      [
                        :get_full_name
                      ])
      it_behaves_like('(the existance of a method returning a boolean value)',
                      [:is_arm_aux_allowed, :is_kick_aux_allowed, :is_body_aux_allowed, :is_breath_aux_allowed])
      it_behaves_like('(the existance of a method returning numeric values)',
                      [:compute_total_distance, :compute_total_seconds])
    end
    #-- -----------------------------------------------------------------------
    #++
  end
