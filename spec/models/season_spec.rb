...
describe Season, type: :model do
  ...
  describe '[a non-valid instance]' do
    ...
    it 'is not a valid season with incorrect edition' do
      expect(build(:season, edition: 1_234_567_890)).not_to be_valid
    end
  ...
  describe '[a well formed instance]' do
    ...
    # Validated relations:
    it_behaves_like('(belongs_to required models)', [:season_type, :timing_type, :edition_type])

    # Filtering scopes:
    it_behaves_like('(the existance of a class method)', [:sort_season_by_begin_date, :sort_season_by_season_type, :sort_season_by_user, :is_not_ended, :is_ended, :is_ended_before, :is_in_range, :for_season_type, :has_results])

    it_behaves_like('(the existance of a method)', [:get_full_name, :get_verbose_name, :is_season_ended_at, :is_season_started_at, :get_season_type, :get_federation_type, :get_last_season_by_type, :build_header_year])
    #-- -----------------------------------------------------------------------
    #++
    ...
    describe 'self.for_season_type' do
      context 'for a SeasonType with existing Seasons,' do
        it 'returns a list of Seasons' do
          season_type = SeasonType.find_by(code: SeasonType::CODE_MAS_CSI)
      ...
      context 'for a SeasonType with NO Seasons,' do
        ...
        it 'returns an empty list' do
          season_type = build(:season_type, code: (Time.now.to_i + 10_000).to_s)
          # DEBUG
    ...
    describe '#get_last_season_by_type' do
      it 'returns the latest season according to current season_type' do
        ...
        seasonnewer = create(
          ...
          edition: subject.edition + 1,
          begin_date: subject.begin_date + 30_065,
          end_date: subject.end_date + 30_065,
          season_type: subject.season_type
        )
