...
RSpec.describe MeetingEventReservation, type: :model do
  ...
  context '[a well formed instance]' do
    ...
    # Validated relations:
    it_behaves_like('(belongs_to required models)', [:meeting, :team, :swimmer, :badge, :meeting_event, :user])
    ...
    context '[general methods]' do
      it_behaves_like('(the existance of a method)', [:suggested_minutes, :suggested_seconds, :suggested_hundreds, :minutes, :seconds, :hundreds, :is_doing_this, :is_not_registered, :is_no_time])
    end
    #-- -----------------------------------------------------------------------
    #++
    ...
    describe '#meeting_program' do
      context "for reservations of 'closed' meetings," do
        ...
        subject { MeetingEventReservation.order('id ASC').limit(100).reject { |e| e.meeting_program.nil? }.min { rand - 0.5 } }
