...
describe Training, type: :model do
  context '[a well formed instance]' do
    subject { Training.find_by(id: ((rand * 10) % Training.count).to_i + 1) }
    ...
    context '[implemented methods]' do
      ...
      it_behaves_like('(the existance of a method returning numeric values)',
                      [:total_distance, :compute_total_distance, :esteemed_total_seconds, :compute_total_seconds])
    end
    #-- -----------------------------------------------------------------------
    #++
  end
  ...
  [:total_distance, :compute_total_distance, :esteemed_total_seconds, :compute_total_seconds].each do |method_name|
