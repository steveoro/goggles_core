...
describe EventType, type: :model do
  ...
  subject { EventType.all.min { rand - 0.5 } }
  ...
  it_behaves_like('(the existance of a method returning a collection of some kind of instances)',
                  [:events_by_pool_types, :pool_types],
                  ActiveRecord::Base)
  # Filtering scopes:
  it_behaves_like('(the existance of a class method)', [:only_relays, :are_not_relays, :for_fin_calculation, :sort_by_style, :for_season, :for_season_type, :for_ironmaster])
  ...
  context 'class methods' do
    describe '#sort_list_by_style_order' do
      let(:fix_list) { %w[200MI 50SL 50FA 100SL 200SL] }
      ...
      it 'returns full event list sorted if invoked without parameter' do
        expect(EventType.sort_list_by_style_order.join(',')).to eq(EventType.sort_by_style.map(&:code).join(','))
      end
      ...
      it 'returns a list correctly sorted if invoked with a list' do
        sorted_list = fix_list.sort { |el_prev, el_next| EventType.find_by(code: el_prev) <=> EventType.find_by(code: el_next) }
