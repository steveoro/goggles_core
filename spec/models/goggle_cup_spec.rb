...
describe GoggleCup, type: :model do
  ...
  describe '[a non-valid instance]' do
    it_behaves_like('(missing required values)', [:description, :season_year, :max_points, :max_performance])
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  describe '[a well formed instance]' do
    ...
    it_behaves_like('(the existance of a method returning a collection of some kind of instances)',
                    [:goggle_cup_definitions, :goggle_cup_standards, :seasons, :meetings, :meeting_individual_results],
                    ActiveRecord::Base)

    # Filtering scopes:
    it_behaves_like('(the existance of a class method)', [:sort_goggle_cup_by_user, :sort_goggle_cup_by_team, :is_closed_now, :is_current, :for_team])

    context '[general methods]' do
      it_behaves_like('(the existance of a method returning non-empty strings)', [:get_full_name, :get_verbose_name])

      it_behaves_like('(the existance of a method returning a date)', [:get_begin_date, :get_end_date])

      it_behaves_like('(the existance of a method returning a boolean value)', [:is_closed_at?, :is_current_at?, :has_results?])
    ...
    describe 'self.has_team_goggle_cup_for_season' do
      ...
      it 'returns false if goggle cup not present' do
        ...
        wrong_team_id   = 12_456
        wrong_season_id = 56_897
    ...
    describe '#calculate_goggle_cup_rank' do
      ...
      context 'for a valid goggle cup' do
        ...
        it 'assigns an array of hash with points to every elements of goggle_cup_rank instance' do
          fix_goggle_cup.calculate_goggle_cup_rank.each do |element|
            expect(element).to be_a_kind_of(Hash)
            expect(element.key?(:total)).to be true
            expect(element.key?(:average)).to be true
            expect(element.key?(:min)).to be true
            expect(element.key?(:max)).to be true
            expect(element.key?(:count)).to be true
          end
        end
