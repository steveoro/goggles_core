...
describe MeetingIndividualResult, type: :model do
  ...
  context '[scope]' do
    describe 'self.sort_by_event_order' do
      it 'does not raise any errors (for ASC sorting)' do
        expect do
          ...
          Meeting.find(17_105).meeting_individual_results.sort_by_event_order.count
        end.not_to raise_error
      end
      it 'does not raise any errors (for DESC sorting)' do
        expect do
          # [Steve, 20180614] Let's choose a meeting w/o too many MIRs and test this scope:
          Meeting.find(17_105).meeting_individual_results.sort_by_event_order('DESC').count
        end.not_to raise_error
      end
    end
    ...
    describe 'self.sort_by_event_and_timing' do
      it 'does not raise any errors (for ASC sorting)' do
        expect do
          # [Steve, 20180614] Let's choose a meeting w/o too many MIRs and test this scope:
          Meeting.find(17_105).meeting_individual_results.sort_by_event_and_timing.count
        end.not_to raise_error
      end
      it 'does not raise any errors (for DESC sorting)' do
        expect do
          # [Steve, 20180614] Let's choose a meeting w/o too many MIRs and test this scope:
          Meeting.find(17_105).meeting_individual_results.sort_by_event_and_timing('DESC').count
        end.not_to raise_error
      end
    end
  end
  ...
  context '[a well formed instance]' do
    ...
    # Validated relations:
    it_behaves_like('(belongs_to required models)', [:meeting_program, :team, :team_affiliation, :badge])
    #:disqualification_code_type  # This field is optional so could be empty

    # Filtering scopes:
    it_behaves_like('(the existance of a class method)', [:is_valid, :is_male, :is_female, :is_disqualified, :is_not_disqualified, :is_personal_best, :has_rank, :for_event_by_pool_type, :for_pool_type, :for_season_type, :for_team, :sort_by_pool_and_event, :sort_by_gender_and_category])

    context '[general methods]' do
      it_behaves_like('(the existance of a method returning non-empty strings)', [:get_full_name, :get_verbose_name, :get_event_by_pool_type_code])
    end
    ...
    context '[methods used for selecting records]' do
      it_behaves_like('(the existance of a method with parameters, returning boolean values)',
                      [:has_pool_type_code?, :has_event_type_code?, :has_category_type_code?, :has_gender_type_code?, :has_federation_type_code?],
                      'fakecode')
    end
    #-- -----------------------------------------------------------------------
    #++
