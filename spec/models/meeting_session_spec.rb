...
describe MeetingSession, type: :model do
  context '[a non-valid instance]' do
    it_behaves_like('(missing required values)', [:description, :session_order, :scheduled_date])
  end
  #-- -------------------------------------------------------------------------
  #++

  shared_examples_for('date/time formatter method') do |method_name, member_name_sym, format_reg_expr, _text_msg_for_not_available|
  ...
  context '[a valid, pre-existing seeded domain]' do
    ...
    subject { Meeting.find_by(id: 13_105).meeting_sessions.last }
