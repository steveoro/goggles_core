...
describe FinCalendar, type: :model do
  describe '[a well formed instance]' do
    ...
    context '[general methods]' do
      ...
      it_behaves_like('(the existance of a method)', [:calendar_year, :calendar_month, :calendar_date, :calendar_name, :calendar_place, :manifest_link, :startlist_link, :results_link, :fin_manifest_code, :fin_startlist_code, :fin_results_code, :goggles_meeting_code, :do_not_update, :get_short_name, :get_full_name, :get_verbose_name, :calendar_unique_key, :get_month_from_fin_code])
    end

    describe 'self.calendar_unique_key' do
      let(:year)    { (2012..2017).to_a.sample }
      let(:rnd_day) { (1..30).to_a.sample }
      let(:month)   { %w[Ottobre Novembre Dicembre Gennaio Febbraio Marzo Aprile Maggio Giugno].sample }
      let(:dates)   { [rnd_day.to_s, "#{rnd_day}-#{rnd_day + 1}"].sample }
