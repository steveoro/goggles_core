...
describe DataImportSwimmerAnalysisResult, type: :model do
  ...
  context '[Standard Factory]' do
    ...
    it_behaves_like('(belongs_to required models)',
                    [:data_import_session, :swimmer, :gender_type, :category_type])

    it_behaves_like('(the existance of a method)',
                    [:analysis_log_text, :sql_text, :searched_swimmer_name, :chosen_swimmer_id, :desired_year_of_birth, :desired_gender_type_id, :category_type_id, :max_year_of_birth, :match_name, :match_score, :best_match_name, :best_match_score, :is_a_perfect_match, :can_insert_alias, :can_insert_swimmer, :rebuild_sql_text, :to_s])
