...
describe MeetingProgram, type: :model do
  ...
  context '[a well formed instance]' do
    ...
    # Validated relations:
    it_behaves_like('(belongs_to required models)', [:meeting_event, :gender_type, :category_type])
    # Filtering scopes:
    it_behaves_like('(the existance of a class method)', [:only_relays, :are_not_relays])

    context '[general methods]' do
      it_behaves_like('(the existance of a method returning non-empty strings)', [:get_short_name, :get_full_name, :get_verbose_name, :get_event_name, :get_complete_event_name, :get_verbose_event_name, :get_category_and_gender_short])
      # Leega. TODO
