...
class EnhanceIndividualRankingDAO
  ...
  class EIRSwimmerScoreDAO
    ...
    def initialize(swimmer, season, total_meetings = season.meetings.count)
      raise ArgumentError, 'Enhance individual ranking swimmer needs a swimmer' unless swimmer&.instance_of?(Swimmer)
      raise ArgumentError, 'Enhance individual ranking swimmer needs a season' unless season&.instance_of?(Season)
      ...
      season.meetings.each do |meeting|
        meeting_individual_results = meeting.meeting_individual_results
          .is_valid.where(
      ...
      @meetings.each_with_index do |meeting, index|
        @total_best_5_on_6 += meeting.get_total_points if total_meetings <= 3 || (index < total_meetings - 1)
      end
    end
    #-- -------------------------------------------------------------------------
    #++
