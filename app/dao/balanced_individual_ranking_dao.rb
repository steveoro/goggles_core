...
class BalancedIndividualRankingDAO
  ...
  class BIREventScoreDAO
    ...
    def initialize(meeting_individual_result, seasonal_event_best, time_converted = nil)
      unless meeting_individual_result&.instance_of?(MeetingIndividualResult)
        raise ArgumentError, 'Balanced individual ranking event score needs a meeting individual result'
      end
      ...
      time_to_use = time_converted || meeting_individual_result.get_timing_instance
  ...
  class BIRMeetingScoreDAO
    ...
    def initialize(meeting, meeting_individual_results, seasonal_event_bests)
      raise ArgumentError, 'Balanced individual ranking meeting score needs a meeting' unless meeting&.instance_of?(Meeting)
      ...
      meeting_individual_results.each do |meeting_individual_result|
        ...
        if meeting_individual_result.pool_type.code == '50'
          time_converted = seasonal_event_bests.timing_converter.convert_time_to_short(
            meeting_individual_result.get_timing_instance,
            meeting_individual_result.gender_type,
            meeting_individual_result.event_type
          )
        end
  ...
  class BIRSwimmerScoreDAO
    ...
    def initialize(swimmer, season, seasonal_event_bests)
      raise ArgumentError, 'Balanced individual ranking swimmer needs a swimmer' unless swimmer&.instance_of?(Swimmer)
      raise ArgumentError, 'Balanced individual ranking swimmer needs a season' unless season&.instance_of?(Season)
      ...
      season.meetings.each do |meeting|
        meeting_individual_results = meeting.meeting_individual_results.is_valid.where(
          ['meeting_individual_results.swimmer_id = ?', @swimmer.id]
        )
      ...
      @meetings.each_with_index do |meeting, index|
        @total_best_5_on_6 += meeting.get_total_points if index < 5
      end
    end
    #-- -------------------------------------------------------------------------
    #++
  ...
  class BIRGenderCategoryRankingDAO
    ...
    def initialize(season, gender_type, category_type, seasonal_event_bests)
      raise ArgumentError, 'Bal.Ind.Ranking needs a season' unless season&.instance_of?(Season)
      raise ArgumentError, 'Bal.Ind.Ranking for gender and category needs a gender type' unless gender_type&.instance_of?(GenderType)
      raise ArgumentError, 'Bal.Ind.Ranking for gender and category needs a category type' unless category_type&.instance_of?(CategoryType)
  ...
  def initialize(season)
    raise ArgumentError, 'Balanced individual ranking needs a season' unless season&.instance_of?(Season)
  ...
  def get_ranking_for_gender_and_category(gender_type, category_type)
    @gender_and_categories.select do |element|
  ...
  def get_html_ranking_description
    'La classifica finale è calcolata considerando le 5 migliori prove su 6.<br>' \
    'Per ogni prova vengono totalizzati i punti in base a:<ul><li>piazzamento</li>' \
    '<li>valore della prestazione rapportata al ranking stagionale</li>' \
    '<li>doppio podio nella manifestazione</li><li>premio gare impegnative</li></ul>'.html_safe
  end
  #-- -------------------------------------------------------------------------
  #++
