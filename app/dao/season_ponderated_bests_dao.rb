...
class SeasonPonderatedBestsDAO
  ...
  class EventPonderatedBestDAO
    ...
    def initialize(season, gender_type, category_type, event_type, pool_type, max_results, bests_to_be_ignored)
      ...
      @best_results        = collect_event_bests
      @ponderated_time     = set_ponderated_best
    end
    #-- -------------------------------------------------------------------------
    #++
  ...
  def to_db!
    ...
    @insert_events.each do |event|
      ...
      comment = "#{event.pool_type.code} #{event.gender_type.code}-#{event.category_type.code} #{event.event_type.code}: #{event.get_ponderated_best}"
