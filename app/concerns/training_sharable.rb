...
module TrainingSharable
  ...
  def visible_to_user( another_user )
    allowed_user_ids = user.friends_sharing_trainings.select(:id).map{ |e| e.id } << user.id
  ...
  module ClassMethods
    ...
    def visible_to_user( any_user )
      allowed_user_ids = any_user.friends_sharing_trainings.select(:id).map{ |e| e.id } << any_user.id
