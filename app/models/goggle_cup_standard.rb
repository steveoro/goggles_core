...
class GoggleCupStandard < ApplicationRecord
  ...
  scope :sort_by_user,        ->(dir) { order("users.name #{dir}, goggle_cups.season_year #{dir}, pool_types.code #{dir}, event_types.code #{dir}, swimmers.complete_name #{dir}") }
  scope :sort_by_goggle_cup,  ->(dir) { order("goggle_cups.season_year #{dir}, pool_types.code #{dir}, event_types.code #{dir}, swimmers.complete_name #{dir}") }
  scope :sort_by_swimmer,     ->(dir) { order("swimmers.complete_name #{dir}, goggle_cups.season_year #{dir}, pool_types.code #{dir}, event_types.code #{dir}") }
  scope :sort_by_event_type,  ->(dir) { order("event_types.code #{dir}, goggle_cups.season_year #{dir}, pool_types.code #{dir}, swimmers.complete_name #{dir}") }
  scope :sort_by_pool_type,   ->(dir) { order("pool_types.code #{dir}, goggle_cups.season_year #{dir}, event_types.code #{dir}, swimmers.complete_name #{dir}") }
