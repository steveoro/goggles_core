...
class NewsFeed < ApplicationRecord
  ...
  validates :title, presence: { length: { within: 1..150 }, allow_nil: false }
  ...
  scope :sort_by_user,            ->(dir) { order("users.name #{dir}, news_feeds.created_at #{dir}") }
  ...
  def set_default_bool_value(field_name_sym, default_value = false)
    send("#{field_name_sym}=".to_sym, default_value) if send(field_name_sym.to_sym).blank?
  end
