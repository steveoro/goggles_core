...
class UserTraining < ApplicationRecord
  ...
  validates :description, length: { within: 1..250, allow_nil: false }
  ...
  def compute_total_distance
    ...
    user_training_rows.without_groups.inject(group_distance) do |sum, row|
      sum + (row.compute_distance.to_i * row.times)
    end
  end
  #-- -------------------------------------------------------------------------
  ...
  def compute_total_seconds
    ...
    # FIXME: USE NEW MEMOIZABLE FIELDS:
    # Start the sum of the rest of the rows using the previous result:
    user_training_rows.without_groups.inject(group_secs) do |sum, row|
      sum + row.compute_total_seconds
    end
  end
  #-- -------------------------------------------------------------------------
