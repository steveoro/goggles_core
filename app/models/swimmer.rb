...
class Swimmer < ApplicationRecord
  ...
  has_many :meeting_event_reservations
  has_many :meeting_relay_reservations
  ...
  scope :sort_by_user,        ->(dir) { order("users.name #{dir}, swimmers.complete_name #{dir}") }
  scope :sort_by_name,        ->(dir = 'ASC') { order("complete_name #{dir}") }
  scope :sort_by_gender_type, ->(dir) { includes(:gender_type).order("gender_types.code #{dir}, swimmers.complete_name #{dir}") }
  #-- -------------------------------------------------------------------------
  #++

  # Computes a shorter description for the name associated with this data
  def get_full_name
    last_name.to_s.empty? ? complete_name.to_s : "#{last_name} #{first_name}"
  end
  ...
  def self.get_team_names(swimmer_id)
    swimmer = Swimmer.find_by(id: swimmer_id)
    swimmer ? swimmer.teams.collect(&:name).uniq : []
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  def get_badge_for(season_id, team_id)
    badges.includes(:season, :team).where(season_id: season_id, team_id: team_id).first
  end
  ...
  def get_badges_array(season_id = nil)
    ...
    #   )
    all_badges = badges.includes(:season, :team)
  ...
  #
  def has_badge_for_season_and_year?(header_year = Season.build_header_year_from_date, season_type = SeasonType.find_by(code: 'MASFIN'))
  ...
  def get_current_category_type_codes
    ...
    curr_fin_category = last_fin_season ? CategoryType.get_category_from(last_fin_season.id, year_of_birth) : nil
    curr_csi_category = last_csi_season ? CategoryType.get_category_from(last_csi_season.id, year_of_birth) : nil
    [curr_fin_category, curr_csi_category].compact.collect(&:code).uniq
  end
  ...
  def get_first_meeting
    ms = meeting_sessions.includes(:meeting).order(:scheduled_date).first
    ms ? ms.meeting : nil
  end

  # Returns the last meeting registered for this Swimmer; nil when not found.
  def get_last_meeting
    ms = meeting_sessions.includes(:meeting).order(:scheduled_date).last
    ms ? ms.meeting : nil
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  def get_disqualifications_count
    (meeting_individual_results.where(is_disqualified: true).count +
      meeting_relay_results.where(is_disqualified: true).count)
  end
  #-- -------------------------------------------------------------------------
  #++
