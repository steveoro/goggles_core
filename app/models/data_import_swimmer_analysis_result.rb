...
#                  :best_match_name, :best_match_score
  ...
  def rebuild_sql_text()
    ...
    if can_insert_swimmer
      self.sql_text << "-- *** WARNING! 'desired_year_of_birth' is not correctly set! ***\r\n" unless self.desired_year_of_birth.to_i > 1900
      self.sql_text << "-- *** WARNING! 'desired_gender_type_id' is not correctly set! ***\r\n" unless self.desired_gender_type_id.to_i == 1 || self.desired_gender_type_id.to_i == 2
...
#                       "#{ self.desired_year_of_birth },#{ self.desired_gender_type_id },1,CURDATE(),CURDATE());\r\n"
      self.sql_text << "INSERT INTO data_import_swimmers (data_import_session_id, import_text, complete_name, year_of_birth, gender_type_id, user_id, created_at, updated_at) VALUES\r\n" <<
                       "    (#{ data_import_session_id }, #{ con.quote( self.searched_swimmer_name ) }, " <<
                       "#{ self.desired_year_of_birth }, #{ self.desired_gender_type_id }, 1, CURDATE(), CURDATE());\r\n"
    end
    if can_insert_alias
