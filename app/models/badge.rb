...
class Badge < ApplicationRecord
  ...
  scope :sort_by_user,          ->(dir)  { joins(:user).order("users.name #{dir}") }
  scope :sort_by_season,        ->(dir)  { joins(:season).order("seasons.begin_date #{dir}") }
  scope :sort_by_team,          ->(dir)  { joins(:team).order("teams.name #{dir}") }
  scope :sort_by_swimmer,       ->(dir)  { joins(:swimmer).order("swimmers.complete_name #{dir}") }
  scope :sort_by_category_type, ->(dir)  { joins(:category_type).order("category_types.code #{dir}") }
