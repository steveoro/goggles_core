...
class ExerciseRow < ApplicationRecord
  ...
  validates :percentage, presence: { length: { within: 1..3 }, allow_nil: false }
  ...
  validates :distance, presence: { length: { within: 1..4 }, allow_nil: false }
  validates :distance, numericality: true
  validates :start_and_rest, presence: { length: { within: 1..4 }, allow_nil: false }
  validates :start_and_rest, numericality: true
  validates :pause, presence: { length: { within: 1..4 }, allow_nil: false }
  ...
  def compute_displayable_distance(total_distance = 0)
    ...
    if distance > 0
      ...
      if total_distance > 0
        (percentage < 100 ? format('%02s', total_distance * percentage / 100).to_s : '')
      else
        (percentage < 100 ? "#{format('%02s', percentage)}%" : '')
      end
    end
  end
  ...
  # - swimmer_level_type_id: the id of the user's swimmer level type (or its
  #   preferred swimmer level type ID); NOT the code, NOT the level: the *ID*;
  #   it can be 0 if it must be ignored
  ...
  def get_full_name(total_distance = 0, verbose_level = :full, swimmer_level_type_id = 0, show_also_ordinal_part = false)
    ...
    when :short
      [
        (show_also_ordinal_part ? format('%02s)', part_order) : ''),
  ...
  # - total_distance: can be 0 if it must be obtained from each component
  # - swimmer_level_type_id: the id of the user's swimmer level type (or its
  #   preferred swimmer level type ID); NOT the code,
  #   NOT the level: the *ID*; it can be 0 if it must be ignored
  ...
  def get_short_description(total_distance = 0, add_movement = :true, add_mode = :true, add_distance = :true)
    [add_distance ? compute_displayable_distance(total_distance) : '',
     ...
     get_formatted_start_and_rest,
     get_formatted_pause].delete_if { |e| e.to_s.empty? }.join(' ')
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  #
  # === Params:
  # - use_i18n_desc: true to use the localized version of the description instead of the computed one;
  #   default to false
  # - swimmer_level_type_id: the id of the user's swimmer level type
  #   (or its preferred swimmer level type ID); NOT the code, NOT the level:
  #    the *ID*; it can be 0 if it must be ignored
  #
