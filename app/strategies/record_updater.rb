...
class RecordUpdater
  ...
  def find_existing_record_for(result_row, must_be_a_team_record)
    record_type = RecordType.find_by(code: must_be_a_team_record ? 'TTB' : 'FOR')
  ...
  def force_update_for_team_records(recordX4d_dao)
    ...
    record_type = RecordType.find_by(code: 'TTB')
    index = 0
    ...
    recordX4d_dao.records.each do |record_dao|
      ...
      index += 1
      @logger.infoc('.') if ((index % LOG_PROGRESS_STEP) == 0) && @logger
    end
  end
  #-- -------------------------------------------------------------------------
  #++
