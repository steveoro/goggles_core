...
class ReservationsCsi2Csv
  ...
  def initialize(meeting, filtering_team = nil, logger = ConsoleLogger.new)
    unless ReservationsCsi2Csv.is_a_csi_meeting(meeting)
      raise ArgumentError, "The specified Meeting must be a valid instance of Meeting, belonging to the '#{SeasonType::CODE_MAS_CSI}' SeasonType."
    end
  ...
  def self.is_a_csi_meeting(meeting)
    ...
    (meeting&.instance_of?(Meeting) && meeting.season_type.code == SeasonType::CODE_MAS_CSI)
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  def output_text
    unless @csi_data_rows.empty?
      @csi_data_rows << @footer_filler.join(';') while @csi_data_rows.size < 400
  ...
  def get_csi_reservation_code(gender_type, category_type, event_type)
    gender_code   = gender_type.get_csi_code.to_i * 10_000
