...
class GoggleCupScoreCalculator
  ...
  def initialize(goggle_cup, swimmer, pool_type, event_type)
    ...
    raise ArgumentError, 'Needs a valid pool type' unless pool_type&.instance_of?(PoolType)
    raise ArgumentError, 'Needs a valid event type' unless event_type&.instance_of?(EventType)
  ...
  def new_goggle_cup_standard(time_swam)
    ...
    comment = "#{@event_type.code}-#{@pool_type.code}: #{time_swam}"
