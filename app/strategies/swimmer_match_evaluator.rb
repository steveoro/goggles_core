...
class SwimmerMatchEvaluator
  ...
  def initialize(local_swimmer)
    raise ArgumentError, 'Swimmer match evaluation needs a valid locale swimmer' unless local_swimmer&.instance_of?(Swimmer)
  ...
  def set_visitor(visitor_swimmer)
    if visitor_swimmer&.instance_of?(Swimmer) &&
