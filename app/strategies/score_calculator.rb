...
class ScoreCalculator
  ...
  def compute_fin_score(time_swam, standard_points = 1000.00, decimals = 2)
    ...
    if time_swam && time_swam.to_hundreds > 0
      ...
      get_time_standard
      fin_score = if @current_time_standard && @current_time_standard.get_timing_instance.to_hundreds > 0
        # Calculate the score with 2 decimals fixed
        @current_time_standard.get_timing_instance.to_hundreds.to_f * standard_points / time_swam.to_hundreds.to_f
      else
  ...
  def desume_time_from_score(goal_score)
    ...
    if goal_score && goal_score > 0
      ...
      if @current_time_standard && @current_time_standard.get_timing_instance.to_hundreds > 0
        ...
        time_to_swim = if goal_score.round(2) == 1000.00
          @current_time_standard.get_timing_instance
        else
          # Calculate the score with 2 decimals fixed
          Timing.new((@current_time_standard.get_timing_instance.to_hundreds.to_f * 1000.00 / goal_score.to_f).to_i)
                       end
      end
    end
    time_to_swim
