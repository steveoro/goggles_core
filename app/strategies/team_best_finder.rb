...
class TeamBestFinder
  ...
  #
  def initialize(team)
    raise ArgumentError, "Needs a valid team: #{team.inspect}" unless team&.instance_of?(Team)
    raise ArgumentError, "Team #{team.get_full_name} hasn't results" unless team.meeting_individual_results.exists?
  ...
  def category_needs_split?(category_type)
    ...
    # variations)
    elsif @distinct_categories.rindex do |e|
